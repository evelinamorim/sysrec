...
class Le{
	...
	void le_tudo(vector<unsigned int>& u,vector<unsigned int>& i,unordered_map< unsigned int,vector<Notas_t> >& n);
	void le_usuarios(vector<unsigned int>& u);
	void le_itens(vector<unsigned int>& i);
	void le_notas(unordered_map< unsigned int,vector< Notas_t> >& n);
