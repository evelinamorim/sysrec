...
void Le::le_tudo(vector<unsigned int>& u,vector<unsigned int>& i,unordered_map< unsigned int,vector< Notas_t> >& n){
    le_usuarios(u);
    le_itens(i);
    le_notas(n);
    //imprime_notas(n);
...
void Le::imprime_notas(unordered_map< unsigned int,vector< Notas_t> > n){
	...
	while(it_second!=it->second.end()){
	    cout<<it->first<<"\t"<<it_second->item_id<<"\t"<<it_second->nota<<"\t"<<it_second->timestamp<<endl;
	    it_second++;
	}
	it++;
    }
...
void Le::le_notas(unordered_map< unsigned int,vector<Notas_t> >& n){
    ...
    if (fd_item.is_open()){
	    ...
	    getline(fd_item,linha);
	    //TODO: conferir se esta pegando as coisas direitas com este separador
	    tokenizar(linha,'\t',tok);
		...
		string is = tok.back();
		tok.pop_back();
		r.item_id = atoi(is.c_str());

		string us = tok.back();
		tok.pop_back();

		n[atoi(us.c_str())].push_back(r);
